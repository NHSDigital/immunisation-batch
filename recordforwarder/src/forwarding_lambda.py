...
def forward_request_to_lambda(message_body):
    ...
    try:
        ...
        update_ack_file(file_key, row_id, successful_api_response=False, diagnostics=error.message, imms_id=None)
    except Exception as error:
        update_ack_file(file_key, row_id, successful_api_response=False, diagnostics=str(error), imms_id=None)
