...
class AppRestrictedAuth:
    def __init__(self, service: Service, cache: Cache):
        ...
        self.token_url = (
            f"https://{environment}.api.service.nhs.uk/oauth2-mock/token"
            if environment != "prod"
            else "https://api.service.nhs.uk/oauth2-mock/token"
        )  # TO CHANGE:oauth2
    ...
    def create_jwt(self, now: int):
        ...
        return jwt.encode(
            claims, secret_object["private_key"], algorithm="RS512", headers={"kid": secret_object["kid"]}
        )
