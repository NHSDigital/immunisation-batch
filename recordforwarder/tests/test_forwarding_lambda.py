...
class TestForwardingLambda(unittest.TestCase):
    ...
    def test_forward_request_to_api_new_duplicate(self):
        with self.common_contexts_for_forwarding_lambda_tests(deepcopy(LAMBDA_PAYLOADS.CREATE.DUPLICATE)):
            forward_request_to_lambda(deepcopy(Message.create_message))
    ...
    def test_forward_request_to_api_update_failure(self):
        ...
        with self.common_contexts_for_forwarding_lambda_tests(mock_lambda_payloads):
    ...
    @patch("forwarding_lambda.sqs_client.send_message")
    def test_forward_request_to_api_update_failure_imms_id_none(self, mock_sqs_message):
        with (
        ...
        mock_sqs_message.assert_called()
