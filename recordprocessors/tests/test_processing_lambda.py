...
class TestProcessLambdaFunction(unittest.TestCase):
    ...
    def test_lambda_handler(
        ...
        with patch.dict(
            ...
            mock_process_csv_to_fhir.assert_called_once_with(
                "source-bucket", "testfile.csv", "Pfizer", "COVID19", "ack-bucket"
            )
    ...
    def test_process_csv_to_fhir(self, mock_csv_dict_reader, mock_send_to_sqs):
        ...
        with patch(
            "processing_lambda.ImmunizationApi.create_imms", return_value=results
        ):
            ...
            process_csv_to_fhir(
                bucket_name, file_key, supplier, "covid19", ack_bucket_name
            )
    ...
    @patch("processing_lambda.send_to_sqs")
    @patch("csv.DictReader")
    def test_process_csv_to_fhir_create_failed(
        self, mock_csv_dict_reader, mock_send_to_sqs
    ):
        s3_client = boto3.client("s3", region_name="us-west-2")
        bucket_name = "test-bucket"
